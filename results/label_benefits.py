...
import pandas as pd
import os
from eval_benefits_prompt import get_benefits_labels 
#get_benefits_labels(model, questions: List[str], answers: List[str], temperature, retry_interval_sec: int = 20)
...
def process_rows(group):
    ...
    for i in range(0, len(group), 10):
        ...
        try:
            json_list, _, _ = get_benefits_labels("gpt-4-0314", prompts_list, responses_list)
            # Write current_scenario_id, context_key_list, question_key_list, and json_list to a CSV file
            with open('benefits_of_punishment_labels.csv', 'a', newline='') as f:
                writer = csv.writer(f)
    ...
    with open('benefits_of_punishment_labels_progress.txt', 'w') as f:
        f.write(str(current_scenario_id))

# Create CSV file if it doesn't exist
if not os.path.exists('benefits_of_punishment_labels.csv'):
    with open('benefits_of_punishment_labels.csv', 'w') as f:
        f.write("scenario_id,context_key,question_key,labels\n")

#Filter for benefits_of_punishments
filtered_data = merged_data[merged_data['question_key'] == 'benefits_of_punishment']

# Read last processed scenario_id from progress file
last_processed_scenario_id = None
if os.path.exists('benefits_of_punishment_labels_progress.txt'):
    with open('benefits_of_punishment_labels_progress.txt', 'r') as f:
